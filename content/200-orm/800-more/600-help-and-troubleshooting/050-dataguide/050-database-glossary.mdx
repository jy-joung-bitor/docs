...
2NF, or second normal form, describes a type of database normalization that:
  1) satisfies the requirements of 1NF, 2) has no values that are tied directly
...
#### 3NF
3NF, or third normal form, describes a type of database normalization that: 1)
  satisfies the requirements of 2NF, 2) each non-key attribute is not
...
4NF, or fourth normal form, describes a type of database normalization that:
  1) satisfies the requirements of BCNF, 2) for every non-trivial multivalued
...
An access control list, often shorted to ACL, is a security policy list that
...
Anti-caching is a strategy that can be used when data is not found in the
  ...
  transaction can be retried later and the information will be ready to served
  from memory.
...
Authorization is an action that determines if a certain user or process should
  ...
  allowed perform what actions. Authorization usually relies on a trusted
...
BASE — an acronym created from the words Basically Available, Soft-state, and
  ...
  databases. It is offered as an description for certain databases that do not
...
BCNF, or Boyce-Codd normal form, describes a type of database normalization
  that: 1) satisfies the requirements of 3NF, 2) where the determining attribute
...
A check constraint is perhaps the most flexible table or column constraint
  ...
  Because of the nature of the condition is fairly open-ended, check constraints
...
Command query responsibility segregation is a application design pattern that
...
Connection pooling is a strategy used to improve performance and avoid
  ...
  time and the database's connection limits can be managed by pooling component.
...
A data type is a category or attribute that expresses a constraint on valid
  ...
  specify expectations and requirements about your data during when defining a
  field or container. The programming language or application can then validate
  that introduced data meets the necessary criteria. Data types are also help
...
Encoding is a system that translates between a character system that can
  ...
  for different many languages (like the UTF unicode varieties).
...
Encrypted transport is any type of communication process that encrypts its
  ...
  different encrypted transport systems can used when deploying databases,
...
Ephemerality is a characteristic that indicates that a piece of data or
  ...
  preventing key leakage from effecting future or past sessions.
...
#### Expand and contract patter>
  ...
  testing, the application can stop writing to original structure and it can be
  removed.
...
A graph database is a NoSQL database that uses a graph structure to store and
  ...
  "teacher". It might be connected to a class node with an edge called that
...
Horizontal scaling, also known as scaling out, is a scaling strategy that
  ...
  complexity in terms of coordination and complexity, especially when data is
  involved.
...
An in-memory database is a database system where the entire data set can all
  be loaded into and processed in the computers memory. This processing model
...
Ingesting data is the act of importing new data into a data system. This can
  ...
  generated by other system. Data ingestion is a common stage of populating and
...
MongoDB is the most popular document-oriented NoSQL database system in use
  ...
  as little complexity as required. MongoDB provides a non-SQL methods and
...
A nonrepeatable read is a type of unwanted consistency problem that can occur
  at certain transaction isolation levels. A nonrepeatable read occurs when a
  repeated read operations within a transaction can return different data based
  based on commits outside of the transaction. This breach of isolation is one
...
A parameterized query, also known as a prepared statement, is a database query
  that has been to take user input as parameters instead of by concatenating
...
#### Pessimistic concurrency contro>
...
Read-through caching is a caching strategy where the cache is deployed in the
  ...
  returned immediately. It the cache request misses, the cache fetches the data
...
#### Read uncommitted isolation leve>
...
A relational database is a database model that organizes data items according
  ...
  relations which describes different operations that can be used to manipulate
...
#### Relational database management system (RDBMS>
  ...
  speaking, an RDBMS manages one or more relational database.

#### Repeatable read isolation leve>
...
#### SQLite
SQLite is a relational management database system written as a C language
...
#### Table aliases
A table alias is name given at query time for an existing or calculated table
...
In natural language processing and full-text search, a token is a discrete
  ...
  relative position in a piece of text, it's type (number, word, phrase, etc.),
...
Write-ahead logging, or WAL, is an approach to data revision management that
  increases the resiliency of systems data corruption during crashes and
  ...
  the data will be in neither the initial nor the intended state. With write
  ahead logging, the system records its intentions to a durable write ahead log
